...
export function formatEntry({ message, prNumber, author }: Entry): string {
  ...
  const messageWithDot = capitalizedMessage.endsWith('.')
    ? capitalizedMessage
    : `${capitalizedMessage}.`;
  return `${messageWithDot} ([#${prNumber}](https://github.com/expo/ncrl-cli/pull/${prNumber}) by [@${author}](https://github.com/${author}))`;
}

const COMMIT_OR_PR_LINK = /\[#?\w+\]\(https:\/\/github\.com\/expo\/ncrl-cli\/(commit|pull)\/\w+\)/;
const COMMIT_OR_PR_LINK_GROUPS =
  /\[(?<number>#?\w+)\]\(https:\/\/github\.com\/expo\/ncrl-cli\/(?<path>(commit|pull)\/\w+)\)/;
