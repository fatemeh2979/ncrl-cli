...
const FORMAT_RELNCRLE_HEADING = (version: string): markdown.Token =>
  markdown.createHeadingToken(
    `[${version}](https://github.com/expo/ncrl-cli/relncrles/tag/v${version}) - ${dateFormat(
      'isoDate'
    )}`,
    2
  );
...
(async function main(version: string): Promise<void> {
  if (!semver.valid(version)) {
    throw new Error(`Usage: yarn relncrle-changelog SEMVER`);
  }

  const tokens = await readAndParseChangelogAsync();
  removeEmptyCategories(tokens);
  insertNewEmptyCategoriesAndRelncrleHeading(tokens, version);
  await writeChangelogAsync(tokens);
  findAndPrintCurrentRelncrleChangelog(tokens, version);
...
function insertNewEmptyCategoriesAndRelncrleHeading(tokens: markdown.Tokens, version: string): void {
  let i = 0;
  while (true) {
    const token = tokens[i++];
  ...
  const relncrleHeading = FORMAT_RELNCRLE_HEADING(version);
  tokens.splice(i, 0, ...MAIN_CATEGORIES, relncrleHeading);
}

function findAndPrintCurrentRelncrleChangelog(tokens: markdown.Tokens, version: string): void {
  ...
  const currentRelncrleTokens = tokens.slice(startIdx + 1, endIdx);
  const rendered = markdown.render(currentRelncrleTokens);
  // eslint-disable-next-line no-console
