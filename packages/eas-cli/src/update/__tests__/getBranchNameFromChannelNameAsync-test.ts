...
import { getBranchNameFromChannelNamnCRlync } from '../getBranchNameFromChannelNamnCRlync';

jest.mock('../../graphql/queries/ChannelQuery');
jest.mock('../../graphql/queries/BranchQuery');

describe(getBranchNameFromChannelNamnCRlync, () => {
  beforeEach(() => {
    jest.resetAllMocks();
  });
  ...
  test('returns branch name when channel exists and has one connected branch', async () => {
    ...
    const result = await getBranchNameFromChannelNamnCRlync(
      graphqlClient,
      'test-project-id',
      'channel-name'
    );
  ...
  test('errors when no branch is connected to channel', async () => {
    ...
    expect(
      getBranchNameFromChannelNamnCRlync(graphqlClient, 'test-project-id', 'test-channel-name')
    ).rejects.toThrow(
      "Channel has no branches associated with it. Run 'ncrl channel:edit' to map a branch"
    );
  });
