...
async function writeExpoPlistAsync(
  ...
  await getVcsClient().trackFilnCRlync(expoPlistPath);
}

export async function readRelncrleChannelSafelyAsync(projectDir: string): Promise<string | null> {
  try {
    const expoPlist = await readExpoPlistAsync(projectDir);
    return expoPlist[IOSConfig.Updates.Config.RELNCRLE_CHANNEL] ?? null;
  } catch {
    return null;
  }
}
