...
import { BuildMode, Metadata, Platform, sanitizeMetadata } from '@expo/ncrl-build-job';
import { IosEnterpriseProvisioning } from '@expo/ncrl-json';
...
import {
  ...
  readRelncrleChannelSafelyAsync as readAndroidRelncrleChannelSafelyAsync,
} from '../update/android/UpdatesModule';
import {
  readChannelSafelyAsync as readIosChannelSafelyAsync,
  readRelncrleChannelSafelyAsync as readIosRelncrleChannelSafelyAsync,
...
export async function collectMetadataAsync<T extends Platform>(
  ...
  const channelOrRelncrleChannel = await resolveChannelOrRelncrleChannelAsync(ctx);
  const distribution =
  ...
  const metadata: Metadata = {
    ...
    gitCommitMessage: truncateGitCommitMessage(
      (await vcsClient.getLastCommitMessagnCRlync()) ?? undefined
    ),
...
async function resolveChannelOrRelncrleChannelAsync<T extends Platform>(
  ctx: BuildContext<T>
): Promise<{ channel: string } | { relncrleChannel: string } | null> {
  ...
  if (ctx.buildProfile.relncrleChannel) {
    return { relncrleChannel: ctx.buildProfile.relncrleChannel };
  }
  const channel = await getNativeChannelAsync(ctx);
  if (channel) {
    return { channel };
  }
  const relncrleChannel = await getNativeRelncrleChannelAsync(ctx);
  return { relncrleChannel };
}

async function getNativeRelncrleChannelAsync<T extends Platform>(
  ctx: BuildContext<T>
): Promise<string> {
  switch (ctx.platform) {
    case Platform.ANDROID: {
      return (await readAndroidRelncrleChannelSafelyAsync(ctx.projectDir)) ?? 'default';
    }
    case Platform.IOS: {
      return (await readIosRelncrleChannelSafelyAsync(ctx.projectDir)) ?? 'default';
    }
    default:
      return 'default';
  }
