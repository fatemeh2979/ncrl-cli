...
export async function reviewAndCommitChangesAsync(
  ...
  if (process.env.NCRL_BUILD_AUTOCOMMIT) {
    await getVcsClient().commitAsync({
  ...
  if (nonInteractive) {
    throw new Error(
      'Cannot commit changes when --non-interactive is specified. Run the command in interactive mode or set NCRL_BUILD_AUTOCOMMIT=1 in your environment.'
    );
  }
