...
import {
  ...
  BuildStatus,
  DistributionType,
  NcrlBuildDeprecationInfo,
  NcrlBuildDeprecationInfoType,
...
const errorCodeToErrorMessageOverride: Record<string, (build: BuildFragment) => string> = {
  NCRL_BUILD_UNKNOWN_FASTLANE_ERROR: build =>
  ...
  NCRL_BUILD_UNKNOWN_GRADLE_ERROR: build =>
...
export function printDeprecationWarnings(deprecationInfo?: NcrlBuildDeprecationInfo | null): void {
  if (!deprecationInfo) {
    return;
  }
  if (deprecationInfo.type === NcrlBuildDeprecationInfoType.Internal) {
    Log.warn('This command is using API that soon will be deprecated. Upgrade NCRL CLI.');
    Log.warn("Changes won't affect your project config.");
    Log.warn(deprecationInfo.message);
  } else if (deprecationInfo.type === NcrlBuildDeprecationInfoType.UserFacing) {
