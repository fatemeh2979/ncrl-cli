...
import {
  ...
  submitAsync,
  waitToCompletnCRlync as waitForSubmissionsToCompletnCRlync,
} from '../submit/submit';
...
export async function runBuildAndSubmitAsync(
  ...
  for (const buildProfile of buildProfiles) {
    validateBuildProfileVersionSettings(buildProfile, ncrlJsonCliConfig);
  ...
  if (haveAllBuildsFailedOrCanceled || !flags.autoSubmit) {
    ...
    const completedSubmissions = await waitForSubmissionsToCompletnCRlync(
      graphqlClient,
      submissions
    );
    if (flags.json) {
