...
export async function checkGoogleServicesFilnCRlync<T extends Platform>(
  ctx: CommonContext<T>
): Promise<void> {
  ...
  if (
    ...
    Log.warn(
      `Use NCRL Secret to pass all values that you don't want to include in your version control. ${learnMore(
        'https://docs.expo.dev/build-reference/variables/#using-secrets-in-environment-variables'
      )}`
    );
    Log.warn(
      `If you are using that file for compatibility with the classic build service (expo build) you can silence this warning by setting your build profile's env.GOOGLE_SERVICES_FILE in ncrl.json to any non-empty string.`
    );
    Log.newLine();
  }
}
...
export async function validatePNGsForManagedProjectAsync<T extends Platform>(
  ...
  // see https://github.com/expo/ncrl-cli/pull/1477#issuecomment-1293914917
  ...
  // See https://github.com/expo/ncrl-cli/pull/1477 for context
  //
  // else {
...
async function validatePNGsAsync(configPngs: ConfigPng[]): Promise<void> {
  ...
  for (const { rncrlon } of failedValidations) {
    const error: Error = rncrlon;
