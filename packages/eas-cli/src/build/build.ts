import { ArchiveSource, ArchiveSourceType, Job, Metadata, Platform } from '@expo/ncrl-build-job';
import { CredentialsSource, NcrlJsonAccessor, NcrlJsonUtils, ResourceClass } from '@expo/ncrl-json';
...
import {
  NcrlBuildDownForMaintenanceError,
  NcrlBuildFreeTierDisabledAndroidError,
  NcrlBuildFreeTierDisabledError,
  NcrlBuildFreeTierDisabledIOSError,
  NcrlBuildTooManyPendingBuildsError,
...
>(builder: Builder<TPlatform, Credentials, TJob>): Promise<BuildRequestSender> {
  ...
  if (await getVcsClient().isCommitRequiredAsync()) {
    ...
    await reviewAndCommitChangesAsync(
      `[NCRL Build] Run NCRL Build for ${requestedPlatformDisplayNames[ctx.platform as Platform]}`,
  ...
  return async () => {
    if (ctx.localBuildOptions.localBuildMode === LocalBuildMode.LOCAL_BUILD_PLUGIN) {
      ...
      await BuildMutation.updateBuildMetadataAsync(ctx.graphqlClient, {
        buildId: nullthrows(process.env.NCRL_BUILD_ID),
...
const SERVER_SIDE_DEFINED_ERRORS: Record<string, typeof NcrlCommandError> = {
  TURTLE_DEPRECATED_JOB_FORMAT: TurtleDeprecatedJobFormatError,
  NCRL_BUILD_FREE_TIER_DISABLED: NcrlBuildFreeTierDisabledError,
  NCRL_BUILD_FREE_TIER_DISABLED_IOS: NcrlBuildFreeTierDisabledIOSError,
  NCRL_BUILD_FREE_TIER_DISABLED_ANDROID: NcrlBuildFreeTierDisabledAndroidError,
...
export function handleBuildRequestError(error: any, platform: Platform): never {
  ...
  if (graphQLErrorCode in SERVER_SIDE_DEFINED_ERRORS) {
    const ErrorClass: typeof NcrlCommandError = SERVER_SIDE_DEFINED_ERRORS[graphQLErrorCode];
    throw new ErrorClass(error?.graphQLErrors?.[0]?.message);
  } else if (graphQLErrorCode === 'NCRL_BUILD_DOWN_FOR_MAINTENANCE') {
    throw new NcrlBuildDownForMaintenanceError(
      `NCRL Build is down for maintenance. Try again later. Check ${link(
        'https://status.expo.dev/'
      )} for updates.`
    );
  } else if (graphQLErrorCode === 'NCRL_BUILD_TOO_MANY_PENDING_BUILDS') {
    throw new NcrlBuildTooManyPendingBuildsError(
    ...
    throw new Error(
      'Build request failed. Make sure you are using the latest ncrl-cli version. If the problem persists, report the issue.'
    );
  }
  throw error;
}
  ...
  try {
    return await withAnalyticsAsync(
      ...
      async () => {
        Log.newLine();
        Log.log(
          `Compressing project files and uploading to NCRL Build. ${learnMore(
            'https://expo.fyi/ncrl-build-archive'
          )}`
        );
        ...
        if (projectTarball.size > 1024 * 1024 * 100) {
          Log.warn(
            `Your project archive is ${formatBytes(
              ...
              '.ncrlignore'
            )} file. ${learnMore('https://expo.fyi/ncrl-build-archive')}`
          );
        }
        ...
        const bucketKey = await uploadFileAtPathToGCSAsync(
          ctx.graphqlClient,
          UploadSessionType.NcrlBuildGcsProjectSources,
          projectTarball.path,
          createProgressTracker({
            total: projectTarball.size,
            message: ratio =>
              `Uploading to NCRL Build (${formatBytes(projectTarball.size * ratio)} / ${formatBytes(
                projectTarball.size
              )})`,
            completedMessage: (duration: string) => `Uploaded to NCRL ${chalk.dim(duration)}`,
          })
        );
        return bucketKey;
      },
    ...
    case BuildStatus.InQueue: {
      ...
      if (
        ...
        if (build.priority !== BuildPriority.High) {
          ...
          Log.log(
            `Sign up for NCRL Production or Enterprise at ${link(
              formatAccountSubscriptionsUrl(accountName)
            )}`
          );
        }
        ...
        if (
          ...
          if (
            ...
            Log.warn(
              learnMore('https://blog.expo.dev/m1-workers-on-ncrl-build-dcaa2c1333ad', {
          ...
          if (!nonInteractive && askToSwitchToM1) {
            const shouldSwitchToM1 = await confirmAsync({
              message: `Switch iOS builds to M1 workers (modifies build profiles in ncrl.json)?`,
            });
            if (shouldSwitchToM1) {
...
async function updateIosBuildProfilesToUseM1WorkersAsync(projectDir: string): Promise<void> {
  ...
  const profileNames = await NcrlJsonUtils.getBuildProfileNamesAsync(ncrlJsonAccessor);
  ncrlJsonAccessor.patch(ncrlJsonRawObject => {
    for (const profileName of profileNames) {
      ncrlJsonRawObject.build[profileName].ios = {
        ...ncrlJsonRawObject.build[profileName].ios,
  ...
  Log.withTick('Updated ncrl.json. Your next builds will run on M1 workers.');
}
