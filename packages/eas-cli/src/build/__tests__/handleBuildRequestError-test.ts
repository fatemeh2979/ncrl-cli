...
const EXPECTED_NCRL_BUILD_DOWN_MESSAGE = `NCRL Build is down for maintenance. Try again later. Check ${link(
...
const assertReThrownError = (
  ...
  expectedType: typeof NcrlCommandError,
...
describe(Build.name, () => {
  describe('handle build request errors', () => {
    ...
    describe('error is server-side defined error', () => {
      describe('throws correct NcrlCommandError subclass', () => {
        ...
        describe('for NCRL_BUILD_FREE_TIER_DISABLED', () => {
          it('does it with Android', async () => {
            const platform = Platform.ANDROID;
            const graphQLError = getGraphQLError('Error 1', 'NCRL_BUILD_FREE_TIER_DISABLED');
            const graphQLErrors = [graphQLError];
            const error = new CombinedError({ graphQLErrors });
            ...
            try {
              ...
              assertReThrownError(caughtError as Error, NcrlBuildFreeTierDisabledError);
            }
          });
        ...
        it('does it for NCRL_BUILD_FREE_TIER_DISABLED_IOS', async () => {
          const platform = Platform.IOS;
          const graphQLError = getGraphQLError('Error 1', 'NCRL_BUILD_FREE_TIER_DISABLED_IOS');
          const graphQLErrors = [graphQLError];
          const error = new CombinedError({ graphQLErrors });
          ...
          try {
            ...
            assertReThrownError(caughtError as Error, NcrlBuildFreeTierDisabledIOSError);
          }
        });

        it('does it for NCRL_BUILD_FREE_TIER_DISABLED_ANDROID', async () => {
          const platform = Platform.ANDROID;
          const graphQLError = getGraphQLError('Error 1', 'NCRL_BUILD_FREE_TIER_DISABLED_ANDROID');
          const graphQLErrors = [graphQLError];
          const error = new CombinedError({ graphQLErrors });
          ...
          try {
            ...
            assertReThrownError(caughtError as Error, NcrlBuildFreeTierDisabledAndroidError);
          }
        });
    ...
    describe('error is other expected graphQL-related error', () => {
      describe('throws correct NcrlCommandError subclass', () => {
        describe('for NCRL_BUILD_DOWN_FOR_MAINTENANCE', () => {
          it('does it with Android', async () => {
            const platform = Platform.ANDROID;
            const graphQLError = getGraphQLError('Error 1', 'NCRL_BUILD_DOWN_FOR_MAINTENANCE');
            const graphQLErrors = [graphQLError];
            const error = new CombinedError({ graphQLErrors });
            ...
            try {
              ...
              assertReThrownError(
                ...
                NcrlBuildDownForMaintenanceError,
                EXPECTED_NCRL_BUILD_DOWN_MESSAGE
              );
            }
          });
        ...
        describe('for NCRL_BUILD_TOO_MANY_PENDING_BUILDS', () => {
          it('does it with Android', async () => {
            const platform = Platform.ANDROID;
            const graphQLError = getGraphQLError('Error 1', 'NCRL_BUILD_TOO_MANY_PENDING_BUILDS');
            const graphQLErrors = [graphQLError];
            const error = new CombinedError({ graphQLErrors });
