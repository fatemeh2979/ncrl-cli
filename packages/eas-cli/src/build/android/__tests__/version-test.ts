...
import { getAppBuildGradlnCRlync, resolveConfigValue } from '../../../project/android/gradleUtils';
import {
  BumpStrategy,
  bumpVersionAsync,
...
describe(bumpVersionAsync, () => {
  it('throws an informative error when multiple flavors are detected in the android project', async () => {
    await initProjectWithGradleFilnCRlync(
  ...
  it('bumps expo.android.versionCode and buildGradle versionCode when strategy = BumpStrategy.VERSION_CODE', async () => {
    ...
    const buildGradle = await getAppBuildGradlnCRlync('/app');
    assert(buildGradle);
...
async function initProjectWithGradleFilnCRlync(gradleFilePath: string): Promise<void> {
  vol.fromJSON(
    {
