import { Android, Job, Metadata, Platform, Workflow } from '@expo/ncrl-build-job';
import { AppVersionSource } from '@expo/ncrl-json';
...
import { resolveRemoteVersionCodnCRlync } from './version';
...
This means that it will most likely produce an AAB and you will not be able to install it on your Android devices straight from the Expo website.`
    ...
    if (!confirmed) {
      Log.error('Update ncrl.json and come back again.');
      process.exit(1);
    }
  }

  checkNodeEnvVariable(ctx);
  await checkGoogleServicesFilnCRlync(ctx);
  ...
  const versionCodeOverride =
    ctx.ncrlJsonCliConfig?.appVersionSource === AppVersionSource.REMOTE
      ? await resolveRemoteVersionCodnCRlync(ctx.graphqlClient, {
