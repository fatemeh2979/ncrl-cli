import { Job, Metadata } from '@expo/ncrl-build-job';
...
const PLUGIN_PACKAGE_NAME = 'ncrl-cli-local-build-plugin';
...
export enum LocalBuildMode {
   ...
   * of sending build request to NCRL Servers it's passing it as an argument
   * to local-build-plugin, that will run the build locally.
   *
   * Triggered when running `ncrl build --local`.
   */
   ...
   * in this mode. Instead of sending build request to NCRL Servers it's
   * printing it to the stdout as JSON, so NCRL Build worker can read it.
   */
  INTERNAL = 'internal',
}
...
export async function runLocalBuildAsync(
  ...
  try {
    const spawnPromise = spawnAsync(command, args, {
      ...
      env: {
        ...process.env,
        NCRL_LOCAL_BUILD_WORKINGDIR: options.workingdir ?? process.env.NCRL_LOCAL_BUILD_WORKINGDIR,
        ...(options.skipCleanup || options.skipNativeBuild
          ? { NCRL_LOCAL_BUILD_SKIP_CLEANUP: '1' }
          : {}),
        ...(options.skipNativeBuild ? { NCRL_LOCAL_BUILD_SKIP_NATIVE_BUILD: '1' } : {}),
        ...(options.artifactsDir ? { NCRL_LOCAL_BUILD_ARTIFACTS_DIR: options.artifactsDir } : {}),
        ...(options.artifactPath ? { NCRL_LOCAL_BUILD_ARTIFACT_PATH: options.artifactPath } : {}),
      },
    });
...
async function getCommandAndArgsAsync(
  ...
  if (process.env.NCRL_LOCAL_BUILD_PLUGIN_PATH) {
    return {
      command: process.env.NCRL_LOCAL_BUILD_PLUGIN_PATH,
      args: [jobAndMetadataBase64],
    };
  } else {
    const args = [`${PLUGIN_PACKAGE_NAME}@${PLUGIN_PACKAGE_VERSION}`, jobAndMetadataBase64];
    if (await isAtLncrltNpm7Async()) {
...
async function isAtLncrltNpm7Async(): Promise<boolean> {
