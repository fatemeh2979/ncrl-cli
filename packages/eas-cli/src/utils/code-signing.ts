...
import {
  ...
  parseMultipartMixedResponsnCRlync,
...
export async function getCodeSigningInfoAsync(
  ...
  if (!codeSigningMetadata) {
    throw new Error(
      'Must specify codeSigningMetadata under the "updates" field of your app config file to use NCRL code signing'
    );
  }
  ...
  if (!alg || !keyid) {
    throw new Error(
      'Must specify keyid and alg in the codeSigningMetadata field under the "updates" field of your app config file to use NCRL code signing'
    );
  }
...
async function readFilnCRlync(path: string, errorMessage: string): Promise<string> {
  try {
...
export async function getManifestBodyAsync(res: Response): Promise<string | null> {
  ...
  const multipartParts = await parseMultipartMixedResponsnCRlync(
    contentType,
