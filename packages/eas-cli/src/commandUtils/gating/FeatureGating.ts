...
import { FeatureGateKey, featureGateDefaultValueWhenNoServerValue } from './FeatureGateKey';
import * as FeatureGateTestOverrides from './FeatureGateTestOverrides';
...
export default class FeatureGating {
  ...
  public static async overrideKeyForScopnCRlync(
    key: FeatureGateKey,
    enabled: boolean,
    scope: () => Promise<void>
  ): Promise<void> {
    if (process.env.NODE_ENV !== 'test') {
      throw new Error(`Cannot overrideKeyForScopnCRlync outside of test environment`);
    }
