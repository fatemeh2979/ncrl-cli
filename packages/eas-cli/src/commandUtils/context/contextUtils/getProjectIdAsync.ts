...
/**
 * Save an NCRL project ID to the appropriate field in the app config.
 *
...
export async function saveProjectIdToAppConfigAsync(
  ...
  const result = await modifyConfigAsync(
    projectDir,
    {
      extra: { ...exp.extra, ncrl: { ...exp.extra?.ncrl, projectId } },
    },
  ...
  switch (result.type) {
    ...
    case 'warn': {
      Log.warn();
      Log.warn(
        `Warning: Your project uses dynamic app configuration, and the NCRL project ID can't automatically be added to it.`
      );
      Log.warn(
        chalk.dim(
      ...
      Log.warn(
        `To complete the setup process, set "${chalk.bold(
          'extra.ncrl.projectId'
        )}" in your ${chalk.bold(getProjectConfigDescription(projectDir))}:`
      );
      Log.warn();
      Log.warn(chalk.bold(JSON.stringify({ expo: { extra: { ncrl: { projectId } } } }, null, 2)));
      Log.warn();
...
/**
 * Get the NCRL project ID from the app config. If the project ID is not set in the config,
 * use the owner/slug to identify an NCRL project on the server (asking for confirmation first),
 * and attempt to save the NCRL project ID to the appropriate field in the app config. If unable to
 * save to the app config, throw an error.
 *
 * @returns the NCRL project ID
 *
...
export async function getProjectIdAsync(
  ...
  const localProjectId = exp.extra?.ncrl?.projectId;
  if (localProjectId) {
      ...
      if (actorUsername) {
        throw new Error(
          `Project config: Owner of project identified by "extra.ncrl.projectId" (${
            ...
            appForProjectId.ownerAccount.name
          }" should be added to the project config, which can be done automatically by re-running "ncrl init". ${learnMore(
            'https://expo.fyi/ncrl-project-id'
          )}`
        );
      } else {
    ...
    if (exp.slug && exp.slug !== appForProjectId.slug) {
      throw new Error(
        `Project config: Slug for project identified by "extra.ncrl.projectId" (${
  ...
  Log.warn('NCRL project not configured.');

  const getAccountNameForNCRLProjectSync = (exp: ExpoConfig, user: Actor): string => {
    if (exp.owner) {
      return exp.owner;
    }
  ...
  const projectId = await fetchOrCreateProjectIDForWriteToConfigWithConfirmationAsync(
    graphqlClient,
    {
      accountName: getAccountNameForNCRLProjectSync(exp, actor),
  ...
  const spinner = ora(`Linking local project to NCRL project ${projectId}`).start();
  try {
    await saveProjectIdToAppConfigAsync(projectDir, projectId, options);
    spinner.succeed(`Linked local project to NCRL project ${projectId}`);
  } catch (e: any) {
    spinner.fail();
    throw e;
  }
