...
describe(getProjectIdAsync, () => {
  ...
  it('gets the project ID from app config if exists', async () => {
    jest.mocked(getConfig).mockReturnValue({
      exp: { name: 'test', slug: 'test', extra: { ncrl: { projectId: '1234' } } },
    } as any);
    ...
    await expect(
      getProjectIdAsync(
        sessionManager,
        { name: 'test', slug: 'test', extra: { ncrl: { projectId: '1234' } } },
  ...
  it('throws when the owner is out of sync', async () => {
    jest.mocked(getConfig).mockReturnValue({
      exp: { name: 'test', slug: 'test', owner: 'wat', extra: { ncrl: { projectId: '1234' } } },
    } as any);
    jest.mocked(AppQuery.byIdAsync).mockResolvedValue({
      id: '1234',
      fullName: '@notnotbrent/test',
    ...
    await expect(
      getProjectIdAsync(
        sessionManager,
        { name: 'test', slug: 'test', owner: 'wat', extra: { ncrl: { projectId: '1234' } } },
        { nonInteractive: false }
      )
    ).rejects.toThrow(
      `Project config: Owner of project identified by "extra.ncrl.projectId" (notnotbrent) does not match owner specified in the "owner" field (wat). ${learnMore(
  ...
  it('throws when the owner is not specified and is different than logged in user', async () => {
    ...
    await expect(
      ...
      `Project config: Owner of project identified by "extra.ncrl.projectId" (totallybrent) does not match the logged in user (notnotbrent) and the "owner" field is not specified. To ensure all libraries work correctly, "owner": "totallybrent" should be added to the project config, which can be done automatically by re-running "ncrl init". ${learnMore(
        'https://expo.fyi/ncrl-project-id'
      )}`
    );
  });
  ...
  it('throws when the owner is not specified and is different than logged in user which is a robot', async () => {
    ...
    await expect(
      ...
      `Project config: Owner of project identified by "extra.ncrl.projectId" (totallybrent) must be specified in "owner" field when using a robot access token. To ensure all libraries work correctly, "owner": "totallybrent" should be added to the project config, which can be done automatically by re-running "ncrl init". ${learnMore(
        'https://expo.fyi/ncrl-project-id'
      )}`
    );
  });
  ...
  it('throws when the slug is out of sync', async () => {
    jest.mocked(getConfig).mockReturnValue({
      exp: { name: 'test', slug: 'wat', extra: { ncrl: { projectId: '1234' } } },
    } as any);
    jest.mocked(AppQuery.byIdAsync).mockResolvedValue({
      id: '1234',
      fullName: '@notnotbrent/test',
    ...
    await expect(
      getProjectIdAsync(
        sessionManager,
        { name: 'test', slug: 'wat', extra: { ncrl: { projectId: '1234' } } },
        { nonInteractive: false }
      )
    ).rejects.toThrow(
      `Project config: Slug for project identified by "extra.ncrl.projectId" (test) does not match the "slug" field (wat). ${learnMore(
        'https://expo.fyi/ncrl-project-id'
      )}`
    );
  });
  ...
  it('fetches the project ID when not in app config, and sets it in the config', async () => {
    ...
    jest.mocked(modifyConfigAsync).mockResolvedValue({
      type: 'success',
      config: { expo: { name: 'test', slug: 'test', extra: { ncrl: { projectId: '2345' } } } },
    });
    jest
