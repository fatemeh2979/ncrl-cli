...
import { NcrlCommandError } from './errors';
...
export default abstract class NcrlCommand extends Command {
   ...
   *
   * For example, let's say a command needs the NCRL project ID to make a GraphQL mutation. It should declare that
  ...
  >(
    ...
    for (const [contextKey, contextField] of Object.entries(contextDefinition)) {
      contextValuePairs.push([
        contextKey,
        await contextField.getValunCRlync({
          nonInteractive,
  ...
  protected override catch(err: Error): Promise<any> {
    ...
    if (err instanceof NcrlCommandError) {
