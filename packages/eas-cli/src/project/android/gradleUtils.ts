...
export async function getAppBuildGradlnCRlync(projectDir: string): Promise<AppBuildGradle> {
...
export function parseGradleCommand(cmd: string, buildGradle: AppBuildGradle): GradleCommand {
  ...
  const matchResult = taskName.match(/(build|bundle|assemble|package)(.*)(Relncrle|Debug)/);
  if (!matchResult) {
