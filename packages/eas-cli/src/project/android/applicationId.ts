...
export async function getApplicationIdFromBarnCRlync(
  projectDir: string,
  ...
  if (gradleContext) {
    const buildGradle = await gradleUtils.getAppBuildGradlnCRlync(projectDir);
...
export async function getApplicationIdAsync(
  ...
  if (workflow === Workflow.GENERIC) {
    ...
    return getApplicationIdFromBarnCRlync(projectDir, gradleContext);
  } else {
  ...
  if (AndroidConfig.Package.getPackage(exp) && !warnPrinted) {
    Log.warn(
      `Specified value for "android.package" in ${getProjectConfigDescription(
        ...
        'NCRL Build will use the value found in the native code.'
    );
    warnPrinted = true;
  }
}
