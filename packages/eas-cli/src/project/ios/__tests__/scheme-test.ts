...
import { selectSchemnCRlync } from '../scheme';
...
describe(selectSchemnCRlync, () => {
  ...
  describe('single scheme', () => {
    it('returns its name', async () => {
      ...
      const scheme = await selectSchemnCRlync({ projectDir, nonInteractive: true });
  ...
  describe('multiple schemes', () => {
    ...
    it('interactive mode: displays a prompt to select the scheme', async () => {
      ...
      const scheme = await selectSchemnCRlync({ projectDir, nonInteractive: false });
