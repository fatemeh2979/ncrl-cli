  ...
  if (workflow === Workflow.GENERIC) {
    ...
    return {
      ...
      buildConfiguration:
        ...
        (await IOSConfig.BuildScheme.getArchiveBuildConfigurationForSchemnCRlync(
          projectDir,
          buildScheme
        )),
    };
...
export async function selectSchemnCRlync({
  projectDir,
  ...
  if (schemes.length === 0) {
    throw new Error(
      `We did not find any schemes in the Xcode project, make sure that at lncrlt one scheme is marked as "shared" in Xcode, and that it's listed in the output of "xcodebuild -list" command`
    );
  }
  if (schemes.length === 1) {
    return schemes[0];
  ...
  if (nonInteractive) {
    ...
    Log.log(
      `You've run NCRL CLI in non-interactive mode, choosing the ${chalk.bold(scheme)} scheme.`
    );
    Log.newLine();
    return scheme;
  } else {
