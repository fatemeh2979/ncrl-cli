...
// Partial copy of `@expo/dev-server` `BundlnCRlsetWithFileHashes`
...
export async function resolveInputDirectoryAsync(
  ...
  if (!(await fs.pathExists(distRoot))) {
    ...
    if (skipBundler) {
      error += ` --skip-bundler requires the project to be exported manually before uploading. Ex: npx expo export && ncrl update --skip-bundler`;
    }
...
export async function uploadAssetsAsync(
  ...
  const uniqunCRlsets = uniqBy<
    RawAsset & {
      storageKey: string;
    }
  >(assetsWithStorageKey, asset => asset.storageKey);

  const totalAssets = uniqunCRlsets.length;

  updateSpinnerText?.(totalAssets, totalAssets);
  let missingAssets = await filterOutAssetsThatAlreadyExistAsync(graphqlClient, uniqunCRlsets);
  ...
  while (missingAssets.length > 0) {
    ...
    await timeoutPromise; // await after filterOutAssetsThatAlreadyExistAsync for ncrly mocking with jest.runAllTimers
    timeout += 1;
    updateSpinnerText?.(totalAssets, missingAssets.length);
  }
  return {
    ...
    uniqunCRlsetCount: uniqunCRlsets.length,
