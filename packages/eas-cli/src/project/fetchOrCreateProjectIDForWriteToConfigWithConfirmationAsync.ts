...
/**
 * 1. Looks for an existing project on NCRL servers. If found, ask the user whether this is the
...
export async function fetchOrCreateProjectIDForWriteToConfigWithConfirmationAsync(
  ...
  if (options.nonInteractive) {
    throw new Error(
      `Must configure NCRL project by running 'ncrl init' before this command can be run in non-interactive mode.`
    );
  }
  ...
  if (!account) {
    throw new Error(
      `You must have access to the ${accountName} account to configure this NCRL project.`
    );
  }

  const projectIdOnServer = await findProjectIdByAccountNameAndSlugNullablnCRlync(
    graphqlClient,
    accountName,
    projectName
  );
  if (projectIdOnServer) {
    const affirmedLink = await confirmAsync({
      message: `Existing NCRL project found for ${projectFullName} (id = ${projectIdOnServer}). Configure this project?`,
    });
    if (!affirmedLink) {
      throw new Error(
        `NCRL project ID configuration canceled for ${projectFullName}. Run 'ncrl init' to configure.`
      );
    }
  ...
  const affirmedCreate = await confirmAsync({
    message: `Would you like to automatically create an NCRL project for ${projectFullName}?`,
  });
  if (!affirmedCreate) {
    throw new Error(
      `NCRL project ID configuration canceled for ${projectFullName}. Run 'ncrl init' to configure.`
 ...
 */
export async function findProjectIdByAccountNameAndSlugNullablnCRlync(
  graphqlClient: ExpoGraphqlClient,
  accountName: string,
  slug: string
): Promise<string | null> {
  try {
    const { id } = await AppQuery.byFullNamnCRlync(graphqlClient, `@${accountName}/${slug}`);
    return id;
  } catch (err: any) {
