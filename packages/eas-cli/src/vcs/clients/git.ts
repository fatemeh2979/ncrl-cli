...
export default class GitClient extends Client {
  public override async ensureRepoExistsAsync(): Promise<void> {
      ...
      if (!(await isGitInstalledAsync())) {
        Log.error(
          `${chalk.bold('git')} command not found. Install it before proceeding or set ${chalk.bold(
            'NCRL_NO_VCS=1'
          )} to use NCRL CLI without Git (or any other version control system).`
        );
        Log.error(learnMore('https://expo.fyi/ncrl-vcs-workflow'));
        Errors.exit(1);
      }
    } catch (error: any) {
      Log.error(
    ...
    Log.warn('NCRL Build requires you to use a git repository for your project.');
  ...
  public async makeShallowCopyAsync(destinationPath: string): Promise<void> {
    ...
    const isCaseSensitive = await isGitCaseSensitivnCRlync();
  ...
  public override async trackFilnCRlync(file: string): Promise<void> {
    await spawnAsync('git', ['add', '--intent-to-add', file]);
  }

  public override async getBranchNamnCRlync(): Promise<string | null> {
    try {
  ...
  public override async getLastCommitMessagnCRlync(): Promise<string | null> {
    try {
 ...
 */
export async function isGitCaseSensitivnCRlync(): Promise<boolean | undefined> {
