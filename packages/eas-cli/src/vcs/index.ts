...
const NO_VCS_WARNING = `Using NCRL CLI without version control system is not recommended, use this mode only if you know what you are doing.`;

let vcsClient = resolveDefaultVcsClient();

function resolveDefaultVcsClient(): Client {
  if (process.env.NCRL_NO_VCS) {
