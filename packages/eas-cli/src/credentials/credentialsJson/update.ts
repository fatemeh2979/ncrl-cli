...
export async function updateAndroidCredentialsAsync(
  ...
  if (!keystore) {
    throw new Error('There are no credentials configured for this project on NCRL servers');
  }
  ...
  await updateFilnCRlync(ctx.projectDir, keystorePath, keystore.keystore);
...
export async function updateIosCredentialsAsync(
  ...
  if (!areAllTargetsConfigured) {
    const errorMessage =
      targets.length === 1
        ? `There are no credentials configured for the ${distributionType} distribution of this project on NCRL servers`
        : `Some of the build targets don't have credentials configured for the ${distributionType} distribution of this project on NCRL servers: ${notConfiguredTargetLabels}`;
...
async function updateFilnCRlync(
  projectDir: string,
  filePath: string,
  base64Data?: string
): Promise<void> {
