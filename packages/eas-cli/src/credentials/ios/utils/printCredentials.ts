...
function sortBuildCredentialsByDistributionType(
  ...
  // The order in which we choose the distribution type from lncrlt to most preferred
  const typePriority = [
...
function displayAscApiKey(
  maybnCRlcApiKey: AppStoreConnectApiKeyFragment | null,
  fields: { label: string; value: string }[]
): void {
  fields.push({ label: 'App Store Connect API Key', value: '' });
  if (maybnCRlcApiKey) {
    const { keyIdentifier, issuerIdentifier, appleTeam, name, roles, updatedAt } = maybnCRlcApiKey;
    fields.push({ label: 'Developer Portal ID', value: keyIdentifier });
    if (name) {
