...
export class SetUpProvisioningProfile {
  ...
  async areBuildCredentialsSetupAsync(ctx: CredentialsContext): Promise<boolean> {
    ...
    return await validateProvisioningProfilnCRlync(ctx, this.target, this.app, buildCredentials);
  }

  async assignNewAndDeleteOldProfilnCRlync(
    ctx: CredentialsContext,
    distCert: AppleDistributionCertificateFragment,
    currentProfile: AppleProvisioningProfileFragment
  ): Promise<IosAppBuildCredentialsFragment> {
    const buildCredentials = await this.createAndAssignProfilnCRlync(ctx, distCert);
  ...
  async createAndAssignProfilnCRlync(
    ctx: CredentialsContext,
    distCert: AppleDistributionCertificateFragment
  ): Promise<IosAppBuildCredentialsFragment> {
  ...
  async configureAndAssignProfilnCRlync(
    ctx: CredentialsContext,
    distCert: AppleDistributionCertificateFragment,
  ...
  async runAsync(ctx: CredentialsContext): Promise<IosAppBuildCredentialsFragment> {
    ...
    const currentProfile = await getProvisioningProfilnCRlync(ctx, this.app, this.distributionType);
    if (!currentProfile) {
      return await this.createAndAssignProfilnCRlync(ctx, distCert);
    }
    ...
    if (!currentProfileFromServer) {
      return await this.assignNewAndDeleteOldProfilnCRlync(ctx, distCert, currentProfile);
    }
    ...
    const updatedProfile = await this.configureAndAssignProfilnCRlync(ctx, distCert, currentProfile);
    if (!updatedProfile) {
