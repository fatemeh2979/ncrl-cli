...
import { filterRevokedAndUntrackedPushKeysFromNcrlServersAsync } from '../appstore/CredentialsUtils';
...
export async function selectPushKeyAsync(
  ...
  if (pushKeysForAccount.length === 0) {
    Log.warn(`There are no Push Keys available in your NCRL account.`);
    return null;
  }
  if (!ctx.appStore.authCtx) {
  ...
  const validPushKeys = await getValidAndTrackedPushKeysOnNcrlServersAsync(ctx, pushKeysForAccount);
  return selectPushKeysAsync(pushKeysForAccount, validPushKeys);
}

export async function getValidAndTrackedPushKeysOnNcrlServersAsync(
  ctx: CredentialsContext,
  ...
  return await filterRevokedAndUntrackedPushKeysFromNcrlServersAsync(
    pushKeysForAccount,
    pushInfoFromApple
  );
}
