...
import { confirmAsync, pressAnyKeyToContinunCRlync, promptAsync } from '../../../prompts';
...
import { validateProvisioningProfilnCRlync } from '../validators/validateProvisioningProfile';
...
export class SetUpAdhocProvisioningProfile {
  ...
  async runAsync(ctx: CredentialsContext): Promise<IosAppBuildCredentialsFragment> {
    ...
    if (areBuildCredentialsSetup) {
      ...
      if (await this.shouldUseExistingProfilnCRlync(ctx, buildCredentials)) {
        return buildCredentials;
      }
    }

    return await this.runWithDistributionCertificatnCRlync(ctx, distCert);
  }

  async runWithDistributionCertificatnCRlync(
    ...
    // 2. Fetch devices registered on NCRL servers
    ...
    if (registeredAppleDevices.length === 0) {
      ...
      if (shouldRegisterDevices) {
        ...
        throw new Error(`Run 'ncrl device:create' to register your devices first`);
      }
    }
    ...
    const provisioningProfileStoreInfo =
      await ctx.appStore.createOrReuseAdhocProvisioningProfilnCRlync(
    ...
    if (currentBuildCredentials?.provisioningProfile) {
      if (
        ...
        appleProvisioningProfile = await ctx.ios.createProvisioningProfilnCRlync(
          ctx.graphqlClient,
          app,
          appleAppIdentifier,
          {
  ...
  private async areBuildCredentialsSetupAsync(ctx: CredentialsContext): Promise<boolean> {
    ...
    return await validateProvisioningProfilnCRlync(ctx, target, app, buildCredentials);
  }

  private async shouldUseExistingProfilnCRlync(
    ctx: CredentialsContext,
  ...
  private async registerDevicesAsync(
    ...
    while (true) {
      if (method === RegistrationMethod.WEBSITE) {
        ...
        await pressAnyKeyToContinunCRlync();
      }
      Log.newLine();
