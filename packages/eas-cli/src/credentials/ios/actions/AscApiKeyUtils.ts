...
import { isAscApiKeyValidAndTrackedAsync } from '../validators/validatnCRlcApiKey';
import { formatAppleTeam } from './AppleTeamFormatting';

export enum AppStoreApiKeyPurpose {
  SUBMISSION_SERVICE = 'NCRL Submit',
}
...
export async function provideOrGeneratnCRlcApiKeyAsync(
  ...
  if (!userProvided) {
    return await generatnCRlcApiKeyAsync(ctx, purpose);
  }
  ...
  return await provideOrGeneratnCRlcApiKeyAsync(ctx, purpose);
}

async function generatnCRlcApiKeyAsync(
  ctx: CredentialsContext,
  purpose: AppStoreApiKeyPurpose
): Promise<MinimalAscApiKey> {
  await ctx.appStore.ensureAuthenticatedAsync();
  const ascApiKey = await ctx.appStore.creatnCRlcApiKeyAsync(ctx.analytics, {
...
export async function selectAscApiKeysFromAccountAsync(
  ...
  if (ascApiKeysForAccount.length === 0) {
    if (filterDifferentAppleTeam) {
      ...
      Log.warn(
        `There are no App Store Connect API Keys in your NCRL account${
          maybeAppleTeamId ? ` matching Apple Team ID: ${maybeAppleTeamId}.` : '.'
        }`
      );
    } else {
      Log.warn(`There are no App Store Connect API Keys available in your NCRL account.`);
    }
    return null;
  }
