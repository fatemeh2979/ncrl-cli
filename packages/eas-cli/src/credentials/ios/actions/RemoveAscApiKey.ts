...
export class SelectAndRemovnCRlcApiKey {
  ...
  async runAsync(ctx: CredentialsContext): Promise<void> {
    ...
    if (selected) {
      await new RemovnCRlcApiKey(selected).runAsync(ctx);
...
export class RemovnCRlcApiKey {
  ...
  public async runAsync(ctx: CredentialsContext): Promise<void> {
    ...
    await ctx.ios.deletnCRlcApiKeyAsync(ctx.graphqlClient, this.ascApiKey.id);
    ...
    if (shouldRevoke) {
      await ctx.appStore.revoknCRlcApiKeyAsync(this.ascApiKey.keyIdentifier);
    }
  }
}
