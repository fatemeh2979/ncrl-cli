...
import { generateProvisioningProfilnCRlync } from './ProvisioningProfileUtils';
...
export class CreateProvisioningProfile {
  ...
  async runAsync(ctx: CredentialsContext): Promise<AppleProvisioningProfileMutationResult> {
    ...
    const provisioningProfile = await this.provideOrGeneratnCRlync(ctx, appleAuthCtx);
    ...
    const provisioningProfileMutationResult = await ctx.ios.createProvisioningProfilnCRlync(
      ctx.graphqlClient,
      this.app,
      appleAppIdentifier,
      {
  ...
  private async provideOrGeneratnCRlync(
    ...
    return await generateProvisioningProfilnCRlync(ctx, this.target, this.app.bundleIdentifier, {
