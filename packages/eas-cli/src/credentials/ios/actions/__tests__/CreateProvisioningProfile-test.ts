...
describe('CreateProvisioningProfile', () => {
  ...
  it('creates a Provisioning Profile in Interactive Mode', async () => {
    ...
    expect(jest.mocked(ctx.appStore.createProvisioningProfilnCRlync).mock.calls.length).toBe(1);
  });
  it('errors in Non Interactive Mode', async () => {
    ...
    expect(jest.mocked(ctx.appStore.createProvisioningProfilnCRlync).mock.calls.length).toBe(0);
  });
});
