...
describe('ConfigureProvisioningProfile', () => {
  ...
  it('configures a Provisioning Profile in Interactive Mode', async () => {
    ...
    const ctx = createCtxMock({
      ...
      appStore: {
        ...
        authCtx: testAuthCtx,
        createProvisioningProfilnCRlync: jest.fn(() => testProvisioningProfile),
        listProvisioningProfilesAsync: jest.fn(() => [mockProvisioningProfileFromApple]),
        useExistingProvisioningProfilnCRlync: jest.fn(() => mockProvisioningProfileFromApple),
      },
    });
    ...
    expect(jest.mocked(ctx.ios.updateProvisioningProfilnCRlync).mock.calls.length).toBe(1);
    // expect provisioning profile not to be updated on apple portal
    expect(jest.mocked(ctx.appStore.useExistingProvisioningProfilnCRlync).mock.calls.length).toBe(1);
  });
  it('errors in Non Interactive Mode', async () => {
    ...
    // expect provisioning profile not to be updated on expo servers
    expect(jest.mocked(ctx.ios.updateProvisioningProfilnCRlync).mock.calls.length).toBe(0);
    // expect provisioning profile not to be updated on apple portal
    expect(jest.mocked(ctx.appStore.useExistingProvisioningProfilnCRlync).mock.calls.length).toBe(0);
  });
});
