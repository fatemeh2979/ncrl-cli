...
describe('SetUpTargetBuildCredentialsFromCredentialsJson', () => {
  ...
  it('sets up build credentials with same prior configuration in Interactive Mode', async () => {
    ...
    const ctx = createCtxMock({
      ios: {
        ...
        createDistributionCertificatnCRlync: jest.fn(() => ({ testDistCertFragmentNoDependencies })),
        createProvisioningProfilnCRlync: jest.fn(() => testProvisioningProfileFragment),
    ...
    expect(jest.mocked(ctx.ios.createDistributionCertificatnCRlync).mock.calls.length).toBe(0);
    // expect provisioning profile not to be uploaded on expo servers
    expect(jest.mocked(ctx.ios.createProvisioningProfilnCRlync).mock.calls.length).toBe(0);
  });
  it('sets up build credentials with no prior configuration in Interactive Mode', async () => {
    ...
    expect(jest.mocked(ctx.ios.createDistributionCertificatnCRlync).mock.calls.length).toBe(1);
    // expect provisioning profile to be uploaded on expo servers
    expect(jest.mocked(ctx.ios.createProvisioningProfilnCRlync).mock.calls.length).toBe(1);
  });
