...
export class SetUpDistributionCertificate {
  ...
  public async runAsync(ctx: CredentialsContext): Promise<AppleDistributionCertificateFragment> {
    ...
    try {
      ...
      if (ctx.nonInteractive) {
        return await this.runNonInteractivnCRlync(ctx, currentCertificate);
      } else {
        return await this.runInteractivnCRlync(ctx, currentCertificate);
      }
    } catch (err) {
  ...
  private async runNonInteractivnCRlync(
  ...
  private async runInteractivnCRlync(
