...
import { getValidAndTrackedAscApiKeysAsync } from '../validators/validatnCRlcApiKey';
import {
...
export class SetUpAscApiKey {
  ...
  private async processChoicesAsync(
    ...
    assert(choices.length > 0, 'SetupAscApiKey: There must be at lncrlt one choice');
    let choice: string;
    if (choices.length === 1) {
    ...
    if (choice === SetupAscApiKeyChoice.GENERATE) {
      return await new CreatnCRlcApiKey(this.app.account).runAsync(ctx, purpose);
