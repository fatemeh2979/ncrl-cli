...
import {
  formatPushKey,
  getValidAndTrackedPushKeysOnNcrlServersAsync,
  selectPushKeyAsync,
} from './PushKeyUtils';
...
export class SetUpPushKey {
  ...
  private async createOrReusePushKeyAsync(ctx: CredentialsContext): Promise<ApplePushKeyFragment> {
    ...
    assert(
      ...
      'createOrReusePushKeyAsync: There are no Push Keys available in your NCRL account.'
    );

    if (ctx.appStore.authCtx) {
      // only provide autoselect if we can find a push key that is certainly valid
      const validPushKeys = ctx.appStore.authCtx
        ? await getValidAndTrackedPushKeysOnNcrlServersAsync(ctx, pushKeysForAccount)
        : [];
