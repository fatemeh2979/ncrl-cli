...
export class SetUpInternalProvisioningProfile {
    ...
    if (!ctx.nonInteractive) {
      if (ctx.appStore.authCtx) {
        if (ctx.appStore.authCtx.team.inHouse) {
          ...
          return await this.setupAdhocProvisioningProfilnCRlync(ctx);
        }
      } else {
        if (adhocBuildCredentialsExist && enterpriseBuildCredentialsExist) {
          ...
          return await this.setupUniversalProvisioningProfilnCRlync(ctx);
        } else {
      ...
      if (adhocBuildCredentialsExist && enterpriseBuildCredentialsExist) {
        throw new Error(
          `You're in non-interactive mode. You have set up both adhoc and universal distribution credentials. Set the 'enterpriseProvisioning' property (to 'adhoc' or 'universal') in ncrl.json to choose the credentials to use.`
        );
      } else if (adhocBuildCredentialsExist) {
        return await this.setupAdhocProvisioningProfilnCRlync(ctx);
      } else if (enterpriseBuildCredentialsExist) {
        ...
        throw new Error(
          `You're in non-interactive mode. NCRL CLI couldn't find any credentials suitable for internal distribution. Run this command again in interactive mode.`
        );
      }
    }
  }

  private async setupAdhocProvisioningProfilnCRlync(
  ...
  private async setupUniversalProvisioningProfilnCRlync(
    ctx: CredentialsContext
  ): Promise<IosAppBuildCredentialsFragment> {
