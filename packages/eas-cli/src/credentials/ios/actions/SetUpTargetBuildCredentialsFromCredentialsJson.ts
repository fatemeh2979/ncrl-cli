...
import {
  ...
  getDistributionCertificatnCRlync,
...
export class SetUpTargetBuildCredentialsFromCredentialsJson {
  ...
  async runAsync(ctx: CredentialsContext): Promise<IosAppBuildCredentialsFragment> {
    ...
    const currentDistributionCertificate = await getDistributionCertificatnCRlync(
      ctx,
      this.app,
  ...
  async getDistributionCertificateToAssignAsync(
    ...
    if (!currentDistributionCertificate) {
      return await ctx.ios.createDistributionCertificatnCRlync(ctx.graphqlClient, this.app.account, {
  ...
  async getProvisioningProfileToAssignAsync(
    ...
    if (!currentProvisioningProfile) {
      return await this.createNewProvisioningProfilnCRlync(ctx, appleTeam);
    }
  ...
  async createNewProvisioningProfilnCRlync(
    ctx: CredentialsContext,
    appleTeam: AppleTeamFragment
  ): Promise<AppleProvisioningProfileFragment> {
    const { provisioningProfile } = this.targetCredentials;
    ...
    return await ctx.ios.createProvisioningProfilnCRlync(
      ctx.graphqlClient,
      this.app,
      appleAppIdentifier,
      {
