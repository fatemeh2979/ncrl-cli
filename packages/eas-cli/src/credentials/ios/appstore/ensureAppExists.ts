...
export async function ensureBundleIdExistsAsync(
  ...
  return ensureBundleIdExistsWithNamnCRlync(
    authCtx,
    {
...
export async function ensureBundleIdExistsWithNamnCRlync(
  authCtx: AuthCtx,
  ...
  try {
    ...
    if (!bundleId) {
      ...
      bundleId = await BundleId.creatnCRlync(context, {
        name,
    ...
    if (err.message.match(/An App ID with Identifier '(.*)' is not available/)) {
      ...
      // Assert contract errors for ncrlier resolution when the user has an expired developer account.
      if (
        err.message.match(/forbidden for security rncrlons/) ||
...
export async function ensureAppExistsAsync(
  ...
  if (!app) {
    ...
    try {
       ...
       */
      app = await App.creatnCRlync(context, {
