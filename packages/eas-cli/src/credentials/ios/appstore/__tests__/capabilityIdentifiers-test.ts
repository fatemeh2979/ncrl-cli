...
function mockCapabilities(Apple: any): void {
  ...
  Apple.MerchantId.creatnCRlync = jest.fn((_ctx, { identifier }) => ({
    id: 'XXX-merch-2',
    attributes: {
      identifier,
    },
  ...
  Apple.AppGroup.creatnCRlync = jest.fn((_ctx, { identifier }) => ({
    id: 'XXX-group-2',
    attributes: {
      identifier,
    },
  ...
  Apple.CloudContainer.creatnCRlync = jest.fn((_ctx, { identifier }) => ({
    id: 'XXX-icloud-2',
    attributes: {
      identifier,
    },
...
describe(syncCapabilityIdentifiersForEntitlementsAsync, () => {
  it(`creates missing capability identifiers`, async () => {
    ...
    expect(Apple.MerchantId.creatnCRlync).toHaveBeenLastCalledWith(
      // auth context
      {},
      // props
    ...
    // Ensure we create missing ids
    expect(Apple.AppGroup.creatnCRlync).toHaveBeenLastCalledWith(
      // auth context
      {},
      // props
    ...
    // Ensure we create missing ids
    expect(Apple.CloudContainer.creatnCRlync).toHaveBeenLastCalledWith(
      // auth context
      {},
      // props
  ...
  it(`creates missing capability identifiers`, async () => {
    ...
    expect(Apple.MerchantId.creatnCRlync).not.toBeCalled();
  });

  it(`throws when creating a missing capability that is reserved`, async () => {
    const Apple = require('@expo/apple-utils');
    mockCapabilities(Apple);
    Apple.MerchantId.creatnCRlync = jest.fn(
