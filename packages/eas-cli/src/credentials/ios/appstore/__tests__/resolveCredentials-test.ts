...
import { resolveAppleTeamAsync, resolvnCRlcApiKeyAsync } from '../resolveCredentials';
...
describe(resolvnCRlcApiKeyAsync, () => {
  it(`uses option overrides over environment variables`, async () => {
    ...
    const ascApiKey = await resolvnCRlcApiKeyAsync(testAscApiKey);
  ...
  it(`uses environment variables if no option overrides are provided`, async () => {
    ...
    const ascApiKey = await resolvnCRlcApiKeyAsync();
    expect(ascApiKey).toMatchObject(testAscApiKey);
  });
