...
async function transformProfilnCRlync(
  cert: Profile,
  authCtx: AuthCtx
...
async function addCertificateToProfilnCRlync(
  ...
  return await profile.regeneratnCRlync();
}

export async function useExistingProvisioningProfilnCRlync(
  authCtx: AuthCtx,
  bundleIdentifier: string,
  ...
  try {
    ...
    const profile = await addCertificateToProfilnCRlync(context, {
...
export async function listProvisioningProfilesAsync(
  ...
  try {
    ...
    const result = await Promise.all(
      profiles.map(profile => transformProfilnCRlync(profile, authCtx))
    );
...
export async function createProvisioningProfilnCRlync(
  ...
  try {
    ...
    const profile = await Profile.creatnCRlync(context, {
    ...
    const result = await transformProfilnCRlync(profile, authCtx);
...
export async function revokeProvisioningProfilnCRlync(
  authCtx: AuthCtx,
  bundleIdentifier: string,
  applePlatform: ApplePlatform,
  profileClass: ProfileClass = ProfileClass.General
  ...
  try {
    ...
    await Promise.all(
      profiles
        ...
        .map(profile => Profile.deletnCRlync(context, { id: profile.id }))
    );
