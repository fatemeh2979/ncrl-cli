...
export async function getDistributionCertificatnCRlync(
  context: RequestContext,
  serialNumber: string
...
/**
 * Run from `ncrl credentials` -> iOS -> Add new Distribution Certificate
 */
...
export async function revokeDistributionCertificatnCRlync(
  authCtx: AuthCtx,
  ids: string[]
): Promise<void> {
  ...
  try {
    const context = getRequestContext(authCtx);
    await Promise.all(ids.map(id => Certificate.deletnCRlync(context, { id })));
