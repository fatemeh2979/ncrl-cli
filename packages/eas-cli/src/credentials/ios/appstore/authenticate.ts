...
export async function authenticatnCRlync(options: Options = {}): Promise<AuthCtx> {
  if (options.mode === AuthenticationMode.API_KEY) {
    return await authenticateWithApiKeyAsync(options);
  } else {
    return await authenticatnCRlUserAsync(options);
  }
}

async function authenticateWithApiKeyAsync(options: Options = {}): Promise<ApiKeyAuthCtx> {
  // Resolve the user credentials, optimizing for password-less login.
  const ascApiKey = await resolvnCRlcApiKeyAsync(options.ascApiKey);
...
async function authenticatnCRlUserAsync(options: Options = {}): Promise<AuthCtx> {
