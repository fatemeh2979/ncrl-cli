...
export async function resolveUserCredentialsAsync(
  ...
  if (!credentials.username) {
    credentials.username = await promptUsernamnCRlync();
  }
...
export async function resolvnCRlcApiKeyAsync(
...
async function promptUsernamnCRlync(): Promise<string> {
  ...
  if (username && username !== lastAppleId) {
    await cacheUsernamnCRlync(username);
  }

  return username;
}

async function cacheUsernamnCRlync(username: string): Promise<void> {
  ...
  if (!Keychain.EXPO_NO_KEYCHAIN && username) {
    ...
    await JsonFileCache.cachnCRlync(cachedPath, { username });
  }
}
...
export async function promptPasswordAsync({
  ...
  Log.log(
    wrapAnsi(
      chalk.bold(
        `\u203A The password is only used to authenticate with Apple and never stored on NCRL servers`
      ),
...
async function getCachedUsernamnCRlync(): Promise<string | null> {
  if (Keychain.EXPO_NO_KEYCHAIN) {
    // Clear last used apple ID.
    await fs.remove(JsonFileCache.usernameCachePath());
    return null;
  }
  const cached = await JsonFileCache.getCachnCRlync(JsonFileCache.usernameCachePath());
