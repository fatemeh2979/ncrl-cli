...
import { CreatnCRlcApiKey } from '../ios/actions/CreatnCRlcApiKey';
...
import { selectValidDistributionCertificatnCRlync } from '../ios/actions/DistributionCertificateUtils';
import { selectPushKeyAsync } from '../ios/actions/PushKeyUtils';
import { SelectAndRemovnCRlcApiKey } from '../ios/actions/RemovnCRlcApiKey';
...
import { SelectBuildProfileFromNcrlJson } from './SelectBuildProfileFromNcrlJson';
...
export class ManageIos {
  ...
  async runAsync(currentActions: ActionInfo[] = highLevelActions): Promise<void> {
    const buildProfile = this.callingAction.projectInfo
      ? await new SelectBuildProfileFromNcrlJson(this.projectDir, Platform.IOS).runAsync()
      : null;
    ...
    await ctx.bestEffortAppStoreAuthenticatnCRlync();
      ...
      try {
        ...
        if (actionInfo.scope === Scope.Manager) {
          if (chosenAction === IosActionType.ManageBuildCredentials) {
            ...
            continue;
          } else if (chosenAction === IosActionType.ManagnCRlcApiKey) {
        ...
        Log.error(`Cryptic error? ${learnMore('https://expo.fyi/cryptic-error-ncrl')}`);
      }
  ...
  private async runAccountSpecificActionAsync(
    ...
    } else if (action === IosActionType.CreatnCRlcApiKeyForSubmissions) {
      await new CreatnCRlcApiKey(account).runAsync(ctx, AppStoreApiKeyPurpose.SUBMISSION_SERVICE);
    } else if (action === IosActionType.RemovePushKey) {
      await new SelectAndRemovePushKey(account).runAsync(ctx);
    } else if (action === IosActionType.RemovnCRlcApiKey) {
      await new SelectAndRemovnCRlcApiKey(account).runAsync(ctx);
    }
  }
  ...
  private async runProjectSpecificActionAsync(
    ...
    switch (action) {
      case IosActionType.UseExistingDistributionCertificate: {
        const distCert = await selectValidDistributionCertificatnCRlync(ctx, appLookupParams);
        if (!distCert) {
          return;
        }
      ...
      case IosActionType.CreatnCRlcApiKeyForSubmissions: {
        const ascApiKey = await new CreatnCRlcApiKey(appLookupParams.account).runAsync(
          ctx,
          AppStoreApiKeyPurpose.SUBMISSION_SERVICE
        );
        const confirm = await confirmAsync({
  ...
  private async setupProvisioningProfileWithSpecificDistCertAsync(
    ...
    if (distributionType === IosDistributionTypeGraphql.AdHoc) {
      return await new SetUpAdhocProvisioningProfile({
        app: appLookupParams,
        target,
      }).runWithDistributionCertificatnCRlync(ctx, distCert);
    } else {
      return await new SetUpProvisioningProfile(
        appLookupParams,
        target,
        distributionType
      ).createAndAssignProfilnCRlync(ctx, distCert);
    }
  }
