...
export class ManageAndroid {
  ...
  async runAsync(currentActions: ActionInfo[] = highLevelActions): Promise<void> {
    ...
    const buildProfile = hasProjectContext
      ? await new SelectBuildProfileFromNcrlJson(this.projectDir, Platform.ANDROID).runAsync()
      : null;
    ...
    while (true) {
      try {
        if (ctx.hasProjectContext) {
          ...
          // copy legacy credentials if user is new to NCRL and has legacy credentials
