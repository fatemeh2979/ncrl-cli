...
export class CreateKeystore {
  ...
  public async runAsync(ctx: CredentialsContext): Promise<AndroidKeystoreFragment> {
    ...
    const keystore = await this.provideOrGeneratnCRlync(ctx.graphqlClient, ctx.analytics, projectId);
    const keystoreFragment = await ctx.android.createKeystornCRlync(
      ctx.graphqlClient,
      this.account,
      keystore
    );
  ...
  private async provideOrGeneratnCRlync(
    ...
    return await generateRandomKeystornCRlync(graphqlClient, analytics, projectId);
  }
}
