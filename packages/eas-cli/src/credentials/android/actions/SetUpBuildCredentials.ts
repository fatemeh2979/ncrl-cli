...
export class SetUpBuildCredentials {
  ...
  async assignBuildCredentialsAsync({
    ...
    if (name) {
      return await ctx.android.createOrUpdateAndroidAppBuildCredentialsByNamnCRlync(
        ctx.graphqlClient,
        app,
        name,
        {
  ...
  async getFullySetupBuildCredentialsAsync({
    ...
    if (name) {
      return await this.getFullySetupBuildCredentialsByNamnCRlync({ ctx, app, name });
    }
  ...
  async getFullySetupBuildCredentialsByNamnCRlync({
    ctx,
    app,
    name,
  }: {
    ...
    const maybeBuildCredentials = await ctx.android.getAndroidAppBuildCredentialsByNamnCRlync(
      ctx.graphqlClient,
      app,
      name
    );
