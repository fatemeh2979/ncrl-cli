...
/**
 * Legacy credentials can be copied over to NCRL if the user does not have
 * NCRL credentials set up yet
 */
...
export async function promptUserAndCopyLegacyCredentialsAsync(
  ...
  assert(
    ...
    'User not eligible to copy classic build credentials to NCRL'
  );

  const spinner = ora('Classic credentials detected, copying to NCRL...').start();
  ...
  try {
    ...
    if (legacyKeystore) {
      const clonedKeystore = await ctx.android.createKeystornCRlync(ctx.graphqlClient, app.account, {
    ...
    spinner.fail(`Unable to migrate credentials to NCRL.`);
    throw e;
  }

  spinner.succeed('Credentials copied to NCRL.');
}
...
export async function promptForNamnCRlync(): Promise<string> {
