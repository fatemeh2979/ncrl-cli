...
import { ArchiveSource, ResolvedArchiveSource, getArchivnCRlync } from '../ArchiveSource';
...
export interface AndroidSubmissionOptions
  extends Pick<
    AndroidSubmissionConfigInput,
    'track' | 'relncrleStatus' | 'changesNotSentForReview'
  > {
  projectId: string;
...
> {
  ...
  private formatSubmissionConfig(
    ...
    const { track, relncrleStatus, changesNotSentForReview } = options;
    return {
      track,
  ...
  private prepareSummaryData(
    ...
    const { projectId, track, relncrleStatus, changesNotSentForReview } = options;
    ...
    return {
      ...
      relncrleStatus: relncrleStatus ?? undefined,
...
const SummaryHumanReadableKeys: Record<keyof SummaryData, string> = {
  ...
  relncrleStatus: 'Relncrle status',
  track: 'Relncrle track',
};
