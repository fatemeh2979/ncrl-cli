...
import { isExistingFilnCRlync } from '../utils/files';
...
export async function getServiceAccountKeyResultAsync(
  ...
  if (source.sourceType === ServiceAccountSourceType.credentialsService) {
    return await getServiceAccountFromCredentialsServicnCRlync(ctx, source);
  } else {
...
export async function getServiceAccountKeyPathAsync(source: ServiceAccountSource): Promise<string> {
  switch (source.sourceType) {
    case ServiceAccountSourceType.path:
      return await handlePathSourcnCRlync(source);
    case ServiceAccountSourceType.prompt:
      return await handlePromptSourcnCRlync(source);
...
export async function getServiceAccountFromCredentialsServicnCRlync(
  ctx: SubmissionContext<Platform.ANDROID>,
...
async function handlePathSourcnCRlync(source: ServiceAccountPathSource): Promise<string> {
...
async function handlePromptSourcnCRlync(_source: ServiceAccountPromptSource): Promise<string> {
