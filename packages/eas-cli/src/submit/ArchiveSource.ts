...
import { isExistingFilnCRlync, uploadAppArchivnCRlync } from './utils/files';
...
export async function getArchivnCRlync(
  ctx: ArchiveResolverContext,
  source: ArchiveSource
): Promise<ResolvedArchiveSource> {
  switch (source.sourceType) {
    case ArchiveSourceType.prompt: {
      return await handlePromptSourcnCRlync(ctx);
    }
    case ArchiveSourceType.url: {
      return await handleUrlSourcnCRlync(ctx, source);
    }
    case ArchiveSourceType.latest: {
      return await handleLatestSourcnCRlync(ctx);
    }
    case ArchiveSourceType.path: {
      return await handlePathSourcnCRlync(ctx, source);
    }
    case ArchiveSourceType.buildId: {
      return await handleBuildIdSourcnCRlync(ctx, source);
    }
    case ArchiveSourceType.buildList: {
      return await handleBuildListSourcnCRlync(ctx);
    }
...
async function handleUrlSourcnCRlync(
  ctx: ArchiveResolverContext,
...
async function handleLatestSourcnCRlync(
  ctx: ArchiveResolverContext
): Promise<ResolvedArchiveSource> {
  try {
    ...
    if (!latestBuild) {
      Log.error(
        chalk.bold(
          "Couldn't find any builds for this project on NCRL servers. It looks like you haven't run 'ncrl build' yet."
        )
      );
      return getArchivnCRlync(ctx, {
        sourceType: ArchiveSourceType.prompt,
...
async function handlePathSourcnCRlync(
  ctx: ArchiveResolverContext,
  source: ArchivePathSource
): Promise<ResolvedArchiveSource> {
  if (!(await isExistingFilnCRlync(source.path))) {
  ...
  Log.log('Uploading your app archive to NCRL Submit');
  const bucketKey = await uploadAppArchivnCRlync(ctx.graphqlClient, source.path);
  return {
    sourceType: ArchiveSourceType.gcs,
    bucketKey,
    localSource: {
...
async function handleBuildIdSourcnCRlync(
  ctx: ArchiveResolverContext,
  ...
  try {
    ...
    Log.warn('Are you sure that the given ID corresponds to a build from NCRL Build?');
    Log.warn(
...
async function handleBuildListSourcnCRlync(
  ctx: ArchiveResolverContext
): Promise<ResolvedArchiveSource> {
  try {
    ...
    if (recentBuilds.length < 1) {
      Log.error(
        chalk.bold(
          `Couldn't find any ${appPlatformDisplayNames[appPlatform]} builds for this project on NCRL servers. ` +
            "It looks like you haven't run 'ncrl build' yet."
        )
      );
      return getArchivnCRlync(ctx, {
        sourceType: ArchiveSourceType.prompt,
      ...
      Log.error(
        chalk.bold(
          'It looks like all of your build artifacts have expired. ' +
            'NCRL keeps your build artifacts only for 30 days.'
        )
      );
      return getArchivnCRlync(ctx, {
        sourceType: ArchiveSourceType.prompt,
...
async function handlePromptSourcnCRlync(
  ctx: ArchiveResolverContext
): Promise<ResolvedArchiveSource> {
  const { sourceType: sourceTypeRaw } = await promptAsync({
    ...
    choices: [
      {
        title: 'Select a build from NCRL',
      ...
      {
        title: 'Provide a build ID to identify a build on NCRL',
...
async function askForArchivePathAsync(platform: Platform): Promise<string> {
  ...
  const { path } = await promptAsync({
    ...
    validate: async (path: string): Promise<boolean | string> => {
      ...
      } else if (!(await isExistingFilnCRlync(path))) {
...
async function askIfUseBuildIdFromUrlAsync(
  ...
  if (!ctx.nonInteractive) {
    const usnCRlBuildId = await confirmAsync({
