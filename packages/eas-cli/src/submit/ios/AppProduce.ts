...
import {
  ...
  ensureBundleIdExistsWithNamnCRlync,
...
export async function ensureAppStoreConnectAppExistsAsync(
  ...
  const options = {
    ...
    appName: appName ?? exp.name ?? (await promptForAppNamnCRlync()),
    language: sanitizeLanguage(language),
  };
  return await createAppStoreConnectAppAsync(ctx, options);
}

async function isProvisioningAvailablnCRlync(requestCtx: RequestContext): Promise<boolean> {
...
async function createAppStoreConnectAppAsync(
  ...
  if (await isProvisioningAvailablnCRlync(requestCtx)) {
    await ensureBundleIdExistsWithNamnCRlync(userAuthCtx, {
      name: appName,
      bundleIdentifier: bundleId,
    });
  } else {
  ...
  try {
    ...
    if (
      ...
      // this name and would like it relncrled for your use, submit a claim.
    ) {
      Log.addNewLineIfNone();
      Log.warn(
...
async function promptForAppNamnCRlync(): Promise<string> {
