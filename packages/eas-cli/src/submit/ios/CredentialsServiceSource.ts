...
export async function getFromCredentialsServicnCRlync(
...
> {
  ...
  if (isAppSpecificPassword) {
    return {
      appSpecificPassword: {
        password: ascOrAsp,
        appleIdUsername: await getAppleIdUsernamnCRlync(ctx),
      },
    };
  } else {
    const ascKeyForSubmissions = nullthrows(
      ascOrAsp.appStoreConnectApiKeyForSubmissions,
      `An NCRL Submit ASC API Key could not be found for ${ascOrAsp.appleAppIdentifier.bundleIdentifier}`
    );
