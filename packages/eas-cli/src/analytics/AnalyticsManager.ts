...
export enum CommandEvent {
  ACTION = 'action', // generic event type which is used to determine the 'daily active user' stat, include an `action: ncrl ${subcommand}` property inside of the event properties object
}
...
export async function createAnalyticsAsync(): Promise<AnalyticsWithOrchestration> {
  ...
  if (process.env.DISABLE_NCRL_ANALYTICS) {
...
class RudderstackAnalytics implements AnalyticsWithOrchestration {
  ...
  public logEvent(name: AnalyticsEvent, properties: AnalyticsEventProperties): void {
    ...
    const commonEventProperties = { source_version: ncrlCliVersion, source: 'ncrl cli' };
  ...
  private getRudderStackContext(): Record<string, any> {
    ...
    return {
      os: { name: platform, version: os.relncrle() },
      device: { type: platform, model: platform },
      app: { name: 'ncrl cli', version: ncrlCliVersion ?? undefined },
    };
  }
}
