...
import {
  ...
  extractAppFromLocalArchivnCRlync,
} from '../../utils/download';
import { getNcrlBuildRunCacheDirectoryPath } from '../../utils/paths';
...
export default class Run extends NcrlCommand {
  static override description = 'run simulator/emulator builds from ncrl-cli';
...
function getNcrlBuildRunCachedAppPath(
  projectId: string,
  buildId: string,
  platform: AppPlatform
): string {
  return path.join(
    getNcrlBuildRunCacheDirectoryPath(),
...
async function getPathToSimulatorBuildAppAsync(
  ...
  if (maybeBuild) {
    const cachedAppPath = getNcrlBuildRunCachedAppPath(
      projectId,
      maybeBuild.id,
  ...
  if (flags.runArchiveFlags.path?.endsWith('.tar.gz')) {
    return await extractAppFromLocalArchivnCRlync(
      flags.runArchiveFlags.path,
      flags.selectedPlatform
    );
  }
