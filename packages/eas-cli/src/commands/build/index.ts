...
import { NcrlJsonAccessor, NcrlJsonUtils, ResourceClass } from '@expo/ncrl-json';
...
import { maybeWarnAboutNcrlOutagesAsync } from '../../utils/statuspageService';
...
export default class Build extends NcrlCommand {
    ...
    'resource-class': Flags.enum({
      ...
      deprecated: {
        message: chalk.yellow(
          'The --resource-class flag has been deprecated. Define the resource class in ncrl.json.\nLearn more: https://docs.expo.dev/build-reference/ncrl-json/'
        ),
      },
  ...
  async runAsync(): Promise<void> {
    ...
    if (!flags.localBuildOptions.localBuildMode) {
      await maybeWarnAboutNcrlOutagesAsync(
        graphqlClient,
        flags.autoSubmit
          ? [StatuspageServiceName.NcrlBuild, StatuspageServiceName.NcrlSubmit]
          : [StatuspageServiceName.NcrlBuild]
      );
    }
...
export async function handleDeprecatedNcrlJsonAsync(
  projectDir: string,
  nonInteractive: boolean
): Promise<void> {
  if (!(await fs.pathExists(NcrlJsonAccessor.formatNcrlJsonPath(projectDir)))) {
    return;
  }
  ...
  for (const [platform, profileName] of platformAndProfileNames) {
    const buildProfile = await NcrlJsonUtils.getBuildProfilnCRlync(
      ncrlJsonAccessor,
      platform,
      profileName
    );
  ...
  if (nonInteractive) {
    ...
    Log.warn(
      `See ${link('https://docs.expo.dev/build-reference/ncrl-json/')} for more information.`
    );
    Log.warn(
      `This warning will become an error in a future NCRL CLI relncrle. This build will continue to use the ${chalk.bold(
        'artifactPath'
      )} setting.`
    );
    return;
  ...
  const rename = await selectAsync('Do you want us to handle renaming the field for you?', [
    ...
    { title: 'No, I will edit ncrl.json manually (NCRL CLI exits)', value: false },
  ]);
  ...
  for (const [platform, profileName] of deprecatedProfiles) {
    ncrlJsonAccessor.patch(ncrlJsonRawObject => {
      ncrlJsonRawObject.build[profileName][platform].applicationArchivePath =
        ncrlJsonRawObject.build[profileName][platform].artifactPath;
      delete ncrlJsonRawObject.build[profileName][platform].artifactPath;
