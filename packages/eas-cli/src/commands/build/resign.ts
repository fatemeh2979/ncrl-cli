...
import { CredentialsSource, NcrlJson, NcrlJsonAccessor, NcrlJsonUtils } from '@expo/ncrl-json';
...
import { NcrlPaginatedQueryFlags } from '../../commandUtils/pagination';
...
export default class BuildResign extends NcrlCommand {
  ...
  async runAsync(): Promise<void> {
    ...
    await maybeWarnAboutNcrlOutagesAsync(graphqlClient, [StatuspageServiceName.NcrlBuild]);
    const ncrlJsonAccessor = new NcrlJsonAccessor(projectDir);
    const ncrlJsonCliConfig: NcrlJson['cli'] =
      (await NcrlJsonUtils.getCliConfigAsync(ncrlJsonAccessor)) ?? {};
