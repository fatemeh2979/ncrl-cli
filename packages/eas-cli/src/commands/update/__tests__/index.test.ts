...
jest.mock('../../../update/getBranchNameFromChannelNamnCRlync');
...
describe(UpdatePublish.name, () => {
  ...
  it('creates a new update with --non-interactive, --channel, and --message', async () => {
    ...
    jest.mocked(getBranchNameFromChannelNamnCRlync).mockResolvedValue('branchFromChannel');
    jest.mocked(ensureBranchExistsAsync).mockResolvedValue({
      branchId: 'branch123',
      createdBranch: false,
    });
...
function mockTestProject({
  ...
  jest
    .spyOn(DynamicProjectConfigContextField.prototype, 'getValunCRlync')
  ...
  jest.spyOn(LoggedInContextField.prototype, 'getValunCRlync').mockResolvedValue({
    actor: jester,
...
function mockTestExport({
  ...
  jest.mocked(uploadAssetsAsync).mockResolvedValue({
    ...
    uniqunCRlsetCount: platforms.length,
