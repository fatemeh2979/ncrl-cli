...
import { ensureNCRLUpdateIsConfiguredAsync } from '../../update/configure';
import { getBranchNameFromChannelNamnCRlync } from '../../update/getBranchNameFromChannelNamnCRlync';
import {
  formatUpdateMessage,
...
export default class UpdatePublish extends NcrlCommand {
  ...
  static override flags = {
    ...
    auto: Flags.boolean({
      description:
        'Use the current git branch and commit message for the NCRL branch and update message',
      default: false,
    }),
  ...
  async runAsync(): Promise<void> {
    ...
    const {
      exp: expPossiblyWithoutNcrlUpdateConfigured,
      projectId,
      projectDir,
    ...
    await maybeWarnAboutNcrlOutagesAsync(graphqlClient, [StatuspageServiceName.NcrlUpdate]);
    ...
    if (channelName) {
      branchName = await getBranchNameFromChannelNamnCRlync(graphqlClient, projectId, channelName);
    }

    if (!branchName) {
      if (autoFlag) {
        branchName = await getDefaultBranchNamnCRlync();
    ...
    if (!updateMessage && autoFlag) {
      updateMessage = (await getVcsClient().getLastCommitMessagnCRlync())?.trim();
    }
    ...
    if (!updateMessage) {
      ...
      ({ updateMessage } = await promptAsync({
        ...
        initial: (await getVcsClient().getLastCommitMessagnCRlync())?.trim(),
    ...
    try {
      ...
      const reusedNormalAssetCount = uploadResults.uniqunCRlsetCount - uploadedNormalAssetCount;
        ...
        if (
          ...
          Log.warn(
            `This update group contains ${uploadedAssetCount} assets and is nearing the server cap of ${assetLimitPerUpdateGroup}.\n` +
              `${learnMore('https://docs.expo.dev/ncrl-update/optimize-assets/', {
  ...
  private sanitizeFlags(flags: RawUpdateFlags): UpdateFlags {
    ...
    if (flags.group || flags.republish) {
      ...
      Log.warn(`  ${chalk.bold([`ncrl update:republish`, ...(args ?? [])].join(' '))}`);
      Log.newLine();
  ...
  return [...new Set(platforms)].map(platform => {
    ...
    return {
      ...
      runtimeVersion: nullthrows(
        ...
        }. ${learnMore('https://docs.expo.dev/ncrl-update/runtime-versions/')}`
      ),
    };
  });
}
