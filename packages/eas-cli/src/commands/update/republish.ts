...
export default class UpdateRepublish extends NcrlCommand {
  ...
  async runAsync(): Promise<void> {
    ...
    const updateMessage = await getOrAskUpdateMessagnCRlync(updatesToPublish, flags);
...
async function getOrAskUpdatesAsync(
  ...
  if (flags.branchName) {
    return await askUpdatesFromBranchNamnCRlync(graphqlClient, {
      ...flags,
  ...
  if (flags.channelName) {
    return await askUpdatesFromChannelNamnCRlync(graphqlClient, {
      ...flags,
...
async function askUpdatesFromBranchNamnCRlync(
  graphqlClient: ExpoGraphqlClient,
  {
    projectId,
    branchName,
...
async function askUpdatesFromChannelNamnCRlync(
  graphqlClient: ExpoGraphqlClient,
  {
    projectId,
    channelName,
  ...
  const branchName = await getBranchNameFromChannelNamnCRlync(graphqlClient, projectId, channelName);
...
async function getOrAskUpdateMessagnCRlync(
