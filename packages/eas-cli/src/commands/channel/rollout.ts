...
async function startRolloutAsync(
  ...
  const branch = await BranchQuery.getBranchByNamnCRlync(graphqlClient, {
    appId: projectId,
    name: branchName,
  });
...
export default class ChannelRollout extends NcrlCommand {
  ...
  async runAsync(): Promise<void> {
     ...
     *    a. incrncrle/decrncrle the rollout percentage.
     *    b. end the rollout.
     */
    ...
    if (!isRollout) {
      rolloutMutationResult = await startRolloutAsync(graphqlClient, {
        channelName,
        branchName: branchName ?? (await promptForBranchNamnCRlync(channelName, nonInteractive)),
        percent,
        nonInteractive,
        projectId,
...
async function promptForBranchNamnCRlync(
  channelName: string,
  nonInteractive: boolean
): Promise<string> {
  if (nonInteractive) {
