...
import { authenticatnCRlync, getRequestContext } from '../../credentials/ios/appstore/authenticate';
import {
...
export default class DeviceDelete extends NcrlCommand {
  ...
  async runAsync(): Promise<void> {
    ...
    if (await this.shouldDisableDeviceOnApplnCRlync(paginatedQueryOptions)) {
      await this.disableDeviceOnApplnCRlync(chosenDevice, appleTeamIdentifier);
    }
  }

  async shouldDisableDeviceOnApplnCRlync({
    nonInteractive,
  ...
  async disableDeviceOnApplnCRlync(
    device: AppleDevice | AppleDeviceQueryResult,
    appleTeamIdentifier: string
  ): Promise<void> {
    const ctx = await authenticatnCRlync({ teamId: appleTeamIdentifier });
    ...
    try {
      ...
      if (appleValidatedDevice) {
        await appleValidatedDevice.updatnCRlync({ status: DeviceStatus.DISABLED });
      }
  ...
  async removeDeviceFromExpoAsync(
    ...
    try {
      await AppleDeviceMutation.deleteAppleDevicnCRlync(graphqlClient, chosenDevice.id);
