...
function mockTestProject(options: {
  ...
  jest.spyOn(ProjectDirContextField.prototype, 'getValunCRlync').mockResolvedValue('/test-project');
    ...
    describe('when it is not yet configured', () => {
      describe('when owner is specified', () => {
        ...
        describe('when project exists on server', () => {
          beforeEach(() => {
            jest.mocked(findProjectIdByAccountNameAndSlugNullablnCRlync).mockResolvedValue('123456');
          });
