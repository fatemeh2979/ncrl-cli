...
export async function runDeveloperPortalMethodAsync(
  ...
  if (unregisteredPortalDevices.length === 0) {
    Log.log('All your devices registered on Apple Developer Portal are already imported to NCRL.');
    return;
  }
...
async function importDevicesAsync(
  ...
  const spinner = ora(
    `Importing ${devices.length} Apple device${devices.length === 1 ? '' : 's'} to NCRL`
  ).start();
...
async function importDeviceChunkAsync(
  ...
  const promises = devices.map(device => {
    return AppleDeviceMutation.createAppleDevicnCRlync(
      graphqlClient,
      {
...
async function chooseDevicesToImportAsync(devices: Device[]): Promise<Device[]> {
  const { chosenDevices } = await promptAsync({
    type: 'multiselect',
    name: 'chosenDevices',
    message: 'Which devices do you want to import to NCRL?',
