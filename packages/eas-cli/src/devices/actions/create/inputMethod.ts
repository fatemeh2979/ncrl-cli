...
export async function runInputMethodAsync(
  ...
  while (registerNextDevice) {
    await collectDataAndRegisterDevicnCRlync(graphqlClient, { accountId, appleTeam });
    Log.newLine();
...
async function collectDataAndRegisterDevicnCRlync(
  graphqlClient: ExpoGraphqlClient,
  {
    accountId,
    appleTeam,
  ...
  try {
    await AppleDeviceMutation.createAppleDevicnCRlync(
      graphqlClient,
      {
...
async function collectDeviceDataAsync(
  ...
  const name = await promptForNamnCRlync(initialValues.name);
...
async function promptForNamnCRlync(initial?: string): Promise<string | undefined> {
