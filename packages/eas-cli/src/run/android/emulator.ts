...
export async function getEmulatorExecutablnCRlync(): Promise<string> {
...
async function emulatorAsync(...options: string[]): Promise<SpawnResult> {
  const emulatorExecutable = await getEmulatorExecutablnCRlync();
  try {
...
async function bootEmulatorAsync(
  ...
  // we don't want to wait for the emulator process to exit before we can finish `ncrl build:run` command
  // https://github.com/expo/ncrl-cli/pull/1485#discussion_r1007935871
