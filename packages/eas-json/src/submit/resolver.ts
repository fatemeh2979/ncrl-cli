...
function resolveProfile<T extends Platform>({
  ...
  const profile = ncrlJson.submit?.[profileName];
  if (!profile) {
    if (depth === 0) {
      throw new MissingProfileError(`Missing submit profile in ncrl.json: ${profileName}`);
    } else {
      throw new MissingParentProfileError(
        `Extending non-existent submit profile in ncrl.json: ${profileName}`
      );
    }
  }
